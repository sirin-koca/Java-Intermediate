...
#### Encapsulation:
Bundling of data and methods that oper* ate on that data into a single unit (class).
Restricts direct access to some of an object's components for safety, achieved using private, protected, and public modifiers.
...
#### Inheritance:
Mechanism by which a new class can inherit properties and behaviors (methods) from a parent class.
Promotes reusability and establishes a relationship between the parent and child classes.
...
#### Association:
Relationship between two classes where one class contains a reference to another.
For instance, a "Car" class might have an "Engine" reference indicating a relationship.

By leveraging these principles, Java offers a structured approach to programming that enhances readability, reusability, and maintainability of code.
